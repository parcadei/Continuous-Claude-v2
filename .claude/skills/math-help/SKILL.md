...
```bash
# Solve equation
opc-run -m runtime.harness scripts/sympy_compute.py \
...
```bash
# Prove commutativity
opc-run -m runtime.harness scripts/z3_solve.py \
...
```bash
# Verify single step
opc-run -m runtime.harness scripts/math_scratchpad.py \
...
opc-run scripts/math_tutor.py steps "x**2 - 5*x + 6 = 0" --operation solve

# Progressive hint (level 1-5)
opc-run scripts/math_tutor.py hint "Solve x**2 - 4 = 0" --level 2

# Generate practice problem
opc-run scripts/math_tutor.py generate --topic algebra --difficulty 2
```
...
```bash
# Matrix operations
opc-run scripts/numpy_compute.py det "[[1,2],[3,4]]"
opc-run scripts/numpy_compute.py inv "[[1,2],[3,4]]"
opc-run scripts/numpy_compute.py eig "[[1,2],[3,4]]"
opc-run scripts/numpy_compute.py svd "[[1,2,3],[4,5,6]]"

# Solve linear system
opc-run scripts/numpy_compute.py solve "[[3,1],[1,2]]" "[9,8]"
```

### SciPy (289 functions)
```bash
# Minimize function
opc-run scripts/scipy_compute.py minimize "x**2 + 2*x" "5"

# Find root
opc-run scripts/scipy_compute.py root "x**3 - x - 2" "1.5"

# Curve fitting
opc-run scripts/scipy_compute.py curve_fit "a*exp(-b*x)" "0,1,2,3" "1,0.6,0.4,0.2" "1,0.5"
```

### mpmath (153 functions, arbitrary precision)
```bash
# Pi to 100 decimal places
opc-run scripts/mpmath_compute.py pi --dps 100

# Arbitrary precision sqrt
opc-run -m scripts.mpmath_compute mp_sqrt "2" --dps 100
```
...
```bash
# 2D plot
opc-run scripts/math_plot.py plot2d "sin(x)" \
    --var x --range -10 10 --output plot.png

# 3D surface
opc-run scripts/math_plot.py plot3d "x**2 + y**2" \
    --xvar x --yvar y --range 5 --output surface.html

# Multiple functions
opc-run scripts/math_plot.py plot2d-multi "sin(x),cos(x)" \
    --var x --range -6.28 6.28 --output multi.png

# LaTeX rendering
opc-run scripts/math_plot.py latex "\\int e^{-x^2} dx" --output equation.png
```
...
opc-run scripts/math_tutor.py hint "integrate x*sin(x)" --level 1

# Get more specific guidance
opc-run scripts/math_tutor.py hint "integrate x*sin(x)" --level 3
```
...
```bash
# Generate problem
opc-run scripts/math_tutor.py generate --topic calculus --difficulty 2

# Get hints progressively
opc-run scripts/math_tutor.py hint "..." --level 1
opc-run scripts/math_tutor.py hint "..." --level 2

# Full solution
opc-run scripts/math_tutor.py steps "..." --operation integrate
```
