...
opc-run scripts/math_router.py route "<user's math request>"
```

## Example Workflow

1. User says: "integrate sin(x) from 0 to pi"
2. You run: `opc-run scripts/math_router.py route "integrate sin(x) from 0 to pi"`
3. Router returns:
   ```json
   {
     "command": "opc-run scripts/sympy_compute.py integrate \"sin(x)\" --var x --lower 0 --upper pi",
     "confidence": 0.95
   }
   ```
...
opc-run scripts/math_router.py list

# List routes by category
opc-run scripts/math_router.py list --category sympy
```
