...
async def gather_fail_fast(
    ...
    """
    Gather coroutines with optional fail-fast using TaskGroup.

    Args:
        coros: List of coroutines to execute in parallel
        fail_fast: If True, use TaskGroup (first exception cancels all).
                   If False, use gather with return_exceptions=True.

    Returns:
        List of results. In fail_fast=False mode, may include Exception objects.

    Raises:
        ExceptionGroup: In fail_fast mode, if any coroutine raises.
    """
    import asyncio
